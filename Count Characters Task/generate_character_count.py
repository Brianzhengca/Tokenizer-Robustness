...
f = open("character_level_prompts.txt", "r", errors="ignore")
lines = []
for line in f.readlines():
    lines.append(line.rstrip())
interface = TransformersInterface(model, vocabulary, tokenizer)
normal_output_file = open("normal_character_outputs.txt", "w")
perturbed_output_file = open("perturbed_character_outputs.txt", "w")
