...
f = open("gsm_prompts.txt", "r", errors="ignore")
lines = []
...
for i in range(1, 6):
    ...
    normal_output_file = open("normal_gsm" + str(length_ratio) + "_outputs.txt", "w")
    perturbed_output_file = open("perturbed_gsm" + str(length_ratio) + "_outputs.txt", "w")
    normal_responses, perturbed_responses, length_ratios = interface.generate_response(lines, max_new_tokens=2048, batch_size=10, target_length = length_ratio)
    ...
    f.close()
    with open("math_length" + str(length_ratio) + "_ratios.pkl", "wb") as f:
